...
func (n *NSQD) Main() error {
	...
	// 确保某些操作只被执行一次
	var once sync.Once
	// 定义一个函数对象，入参是error。函数的作用是err不为nil，则将err传递给exitCh
	// 这函数相当于封装了goroutine之间的通信窗口
	...
	// 在一个goroutine中启动一个TCPServer
	n.waitGroup.Wrap(func() {
		exitFunc(protocol.TCPServer(n.tcpListener, n.tcpServer, n.logf))
	})
	// 启动一个httpServer，即nsqlookup
	if n.httpListener != nil {
	...
	// 利用概率算法对优先级队列进行处理，并根据一定的策略来决定是否继续执行循环或休眠。
